...
public class DiningHakkersOnBecome {
   ...
   */
   public static class Hakker extends AbstractActor {
    ...
    PartialFunction<Object, BoxedUnit> waiting_for(ActorRef chopstickToWaitFor, ActorRef otherChopstick) {
      return ReceiveBuilder.
    ...
    PartialFunction<Object, BoxedUnit> denied_a_chopstick = ReceiveBuilder.
      match(Taken.class, t -> {
    ...
    PartialFunction<Object, BoxedUnit> hungry = ReceiveBuilder.
      match(Taken.class, t -> t.chopstick == left,
        t -> context().become(waiting_for(right, left))).
      match(Taken.class, t -> t.chopstick == right,
        t -> context().become(waiting_for(left, right))).
      match(Busy.class,
        b -> context().become(denied_a_chopstick)).
      build();
